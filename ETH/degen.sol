...
abstract contract Router {
    ...
    function getBestQuote(address[] memory path, uint256 amountIn, OrderType orderType, uint256 disableFlags) public view returns (uint, uint256,uint256[] memory) {
        ...
        function(uint256, address[] memory, OrderType ,uint256 ) view returns(uint256,uint256[]memory)[5] memory reserves = [
            disableFlags.disabled(FLAG_1INCH)   ? _calculateNoReturn : calculate1InchReturn,
	    ...
	    disableFlags.disabled(FLAG_Paraswap)   ? _calculateNoReturn : calculateParaSwapReturn,
	    disableFlags.disabled(FLAG_Curve)   ? _calculateNoReturn : calculateCurveReturn,		
        ];
...
contract Degen is Router, Ownable {
    ...
    address _sushi = address(0xd9e1cE17f2641f24aE83637ab66a2cca9C378B9F); // Mainnet network address for sushiswap
    address USDT = address(0xdAC17F958D2ee523a2206206994597C13D831ec7); // USDT Token Address
    ...
    Reimbursement public reimbursementContract;      // reimbursement contract address
